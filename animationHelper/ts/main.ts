...
let frame = { w: 16, h: 16, c: 1, l: <HTMLImageElement[]>[] };
...
function splitImg()
{
	...
	{
		loaded += 1;
		if (frame.l.length <= loaded) resolveF();
	}
...
function redraw(origSize=false)
{
	...
	{
		let imgI = i;
		if (!isNaN(inp_shift.valueAsNumber)) imgI = (imgI + inp_shift.valueAsNumber + frame.l.length) % frame.l.length;
		let img = frame.l[imgI];
		if (inp_reverse.checked) img = frame.l[frame.l.length - imgI - 1];
		ctx.save()
