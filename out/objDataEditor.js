            ...
            TD([], [
                Button("delete-button", "Удалить", async () => {
                    ...
                    if (obj instanceof Decor)
                        text = "Вы уверены, что хотите удалить декорацию?";
...
class EntityEditor_TileSeclector {
    ...
    cursor;
    numered;
    constructor(vx, vy, oneTile, cur, numered) {
        this.popup = new Popup();
        this.view = world.map[vy][vx];
        this.numered = !!numered;
        this.oneTile = oneTile;
        if (cur)
            this.selected = JSON.parse(JSON.stringify(cur));
        if (oneTile)
            this.popup.title = "Выбор клетки";
        else
            this.popup.title = "Выбор клеток";
    ...
    draw() {
        if (this.view == undefined)
            return;
        const drawTile = (x, y, shadow = false) => {
            if (this.view == undefined)
                return;
