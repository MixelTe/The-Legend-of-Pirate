...
15. ## Класс Entity
	...
	* Методы:
		...
		13. canPassThrough(entity: Entity) -> bool - может ли сущность пройти сквозь данную
		14. tileSpeed(tile: Tile) -> float - скорость сущности на клетке
		15. get_tile(dx: int = 0, dy: int = 0, pos: tuple[float, float] = (0.5, 0.5)) -> tuple[Tile, tuple[int, int]] | tuple[None, None] - клетка относительно сущности и её координаты. pos - позиция точки проверки в сущности, где 0 - левый верхний угол, 1 - правый нижний
		16. get_entities(rect: tuple[float, float, float, float]) -> list\[Entity] - сущности попадающие в область
		17. get_entitiesD(rect: tuple[float, float, float, float]) -> list\[Entity] - сущности попадающие в область, относительную сущности
...
19. ## Класс EntityPlayer(EntityAlive)
	* Поля:
		...
		* messageIsLong: bool - если True, в overlay рисуется иконка, обозначающая, что у сообщения есть продолжение (при нажатии кнопки действия)
		* visibleForEnemies: bool - виден ли игрок для противников
		* saveData: SaveData
	...
	* Методы:
		...
		* takeItemFun(size: tuple[float, float], drawFun: (pygame.Surface, float, float, float) => None, onAnimEnd: () => None = None) - активировать анимацию взятия предмета, для отображения будет использоваться drawFun с параметрами поверхность, x, y, размер объекта (0-1). По завершению анимации будет вызвана функция onAnimEnd, если передана.
---
20. ## Класс Animator
	* Аниматор сущностей
	* Поля:
